...
function NFTDropPage({collection}:Props) {
    ...
    useEffect(()=> {
        ...
        const fetchNFTDropData = async () => {
            const allClaimed = await nftDrop.getAllClaimed();
            const getTotal = await nftDrop.totalSupply();
            setClaimed(allClaimed.length);
            setTotal(getTotal);
        }
        fetchNFTDropData();
        ...
        <div className='flex flex-1 p-12 flex-col lg:col-span-6 items-start justify-start'>
            ...
            <div className='w-full mt-10 lg:justify-center flex flex-1 flex-col items-center space-y-4 text-center'>
                <img
                 ...
                 >{collection.title}</h1>
                <p className='text-green-500 text-lg pt-2'>
                     {claimed} / {total?.toString()} NFT's Claimed</p>
            </div>
